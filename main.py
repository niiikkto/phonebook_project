...
from utils import display_menu, validate_email, validate_phone
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            confirm = input(
                f"Вы уверены, что хотите удалить '{name}'? (да/нет): "
            ).lower()
            if confirm == "да":
            ...
            new_phone = input(
                f"Новый телефон (было: {phonebook.find_contact(name).phone}): "
            ).strip()
            ...
            new_email = input(
                f"Новый email (было: {phonebook.find_contact(name).email}): "
            ).strip()
            ...
            new_address = input(
                f"Новый адрес (было: {phonebook.find_contact(name).address}): "
            ).strip()
